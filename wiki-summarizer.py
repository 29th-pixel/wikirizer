...
def generate_summary(url, top_n=5):
    ...
    ranked_sentence = sorted(
        ((scores[i] if i != 0 else 1, s) for i, s in enumerate(sentences)), reverse=True)
...
def exportPDF(message):
    ...
    try:
        pdf.cell(
            200, 7.5, txt=f"Summary - {handle_text_doc.title}", ln=1, align='C')
        ...
        bot.send_message(
            message.chat.id, "Can't export this summary to PDF due to decoding error")
...
def exportImage(message):
    ...
    try:
        line_width, line_height = Font.getsize(
            f"Summary - {handle_text_doc.title}")
        draw.text(((1920 - line_width) / 2, y_text),
                  f"Summary - {handle_text_doc.title}", font=Font, fill=(0, 0, 0))
        y_text += line_height
    except:
        bot.send_message(message.chat.id, "Error No URL is passed")
    else:
        ...
        for text in handle_text_doc.splitted[0]:
            ...
            for line in lines:
                if (y_text + line_height < 1080):
                    ...
                    draw2.text(((1920 - line_width2) / 2, y_text2),
                               line, font=Font, fill=(0, 0, 0))
...
def handle_text_doc(message):
    ...
    try:
        ...
        if (valid_site(url)):
            handle_text_doc.splitted = telebot.util.split_string(
                generate_summary(url, 3), 3000)
            handle_text_doc.title = get_title(url)
            bot.send_message(
                message.chat.id, f"Summary of {handle_text_doc.title}:")
            ...
            bot.send_message(
                message.chat.id, "Your Link is broken please check it")
